...
class MLCircuitBreaker {
  constructor() {
    ...
    this.FAILURE_THRESHOLD = 3 // Abrir circuit após 3 falhas 429
    this.RECOVERY_TIMEOUT = 60000 // 1 minuto para tentar recovery
  ...
  async on429Error(mlAccountId, retryAfterSeconds = null) {
    ...
    if (state.failures >= this.FAILURE_THRESHOLD || state.state === 'HALF_OPEN') {
      // Calcular backoff time
      let backoffTime = this.RECOVERY_TIMEOUT

      // Se ML enviou Retry-After, usar esse tempo
