 ...
 * Eventos para streaming token-by-token do agente Gemini
 */

/**
 * Emit agent token durante geração da resposta
 */
async function emitAgentToken(questionId, organizationId, token, sequenceNumber) {
  console.log('🔥 [EmitEvents] PUBLISHING agent:token to Redis', {
    organizationId,
    questionId,
  ...
  await publishEvent('agent:token', eventData)

  console.log('✅ [EmitEvents] Published agent:token successfully')
}

/**
 * Emit agent step (progresso do workflow)
 */
async function emitAgentStep(questionId, organizationId, step, stepData) {
  const eventData = {
    organizationId,
    questionId,
    type: 'step',
  ...
  await publishEvent('agent:step', eventData)
}

/**
 * Emit agent done (resposta completa)
 */
async function emitAgentDone(questionId, organizationId, response, confidence, processingTime, tokensUsed) {
  const eventData = {
    organizationId,
    questionId,
    type: 'done',
  ...
  await publishEvent('agent:done', eventData)
}

/**
 * Emit agent error
 */
async function emitAgentError(questionId, organizationId, error, errorCode) {
  const eventData = {
    organizationId,
    questionId,
    type: 'error',
  ...
  await publishEvent('agent:error', eventData)
}

/**
 * Emit agent confidence update
 */
async function emitAgentConfidence(questionId, organizationId, confidence) {
  const eventData = {
    organizationId,
    questionId,
    type: 'confidence',
  ...
  await publishEvent('agent:confidence', eventData)
}
